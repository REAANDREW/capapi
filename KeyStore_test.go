...
func TestKeyStores(t *testing.T) {
	...
	var cassandraKeyStore = NewCassandraKeyStore([]string{"0.0.0.0:9042"})
	defer cassandraKeyStore.Stop()
	cassandraKeyStore.Start()

	keyStores := map[string]KeyStore{
		"inproc":    CreateInProcKeyStore(),
		"cassandra": cassandraKeyStore,
	}

	for key, keyStore := range keyStores {
		Convey(fmt.Sprintf("KeyStore Implementation: %s", key), t, func() {
			Convey("Returns", func() {
